...
Reduce is the multi-tool. Unlike Map and Filter, Reduce wants an object. The object is a starting point.
...
The `0` in this instance is the starting place of sum.

## Advanced Reduce

Reduce takes all of the values and returns a single object. In the case above it was the sum, starting from 0. But it can also start with an empty Array or an empty Object and you can transform the data into any way you see fit. Here is the example from the video:

He used tabbed data similar to several lines of this:

"mark johanseen  waffle iron 80  2"

And used this to turn it into an object with internal arrays:
...
.reduce((customers, line)=> {
 ...
 // data prior to reduce:

 [
     ["mark johanssen", "waffle iron", "80", "2"]
 ]
...
function makeNewProp(obj, propName, val){
    return obj[propName] = val;
}
makeNewProp(data, 'last', 'seymour');
console.log(data.last === 'seymour');
console.log(data.propName === 'seymour');
```

Let's walk through this. To me, all of it makes sense except for the reduce method, so I'm going to start there. 

* `customers` is the empty object. 
* `line` is the array inside of the arrays generated by two split methods.
* `customers[line[0]]` is object notation. It is dynamically creating a key value for the object `customers`
